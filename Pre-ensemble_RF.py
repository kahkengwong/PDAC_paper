...
def run_analysis():
    ...
    # Best genes and hyperparameters for RF
    RF_best_gene_list = ["COX6B1", "TMEM258", "IFI27L2", "MYL6", "SRP14", "TNFAIP2", "GNG5", "DSTN", "FLOT1", "RNF181"]
    best_params = {'criterion': 'gini', 'max_depth': 26, 'max_leaf_nodes': 890, 'min_impurity_decrease': 0.01721181971315459, 'min_samples_leaf': 0.06144893511212497, 'min_samples_split': 0.0984787824493502, 'n_estimators': 10}
    
    # Train the best model using the best parameters from TPE
    RF_best_model = RandomForestClassifier(random_state=1, **best_params)
    RF_best_model.fit(normalized_data[RF_best_gene_list].values, normalized_data['status'])
    
    # Prediction and evaluation on the training set
    X_train = normalized_data[RF_best_gene_list].values
    y_train = normalized_data['status']
    predicted_status_train = RF_best_model.predict(X_train)
    predicted_probs_train = RF_best_model.predict_proba(X_train)[:, 1] 
    normalized_data['Probability'] = predicted_probs_train

    # Prediction and evaluation on the test set
    X_test_train = normalized_test_data[RF_best_gene_list].values
    y_test_train = normalized_test_data['status']
    test_set_predictions = RF_best_model.predict(X_test_train)
    test_set_pred_probs = RF_best_model.predict_proba(X_test_train)[:, 1]
    ...
    calibrated_model = CalibratedClassifierCV(RF_best_model, method='sigmoid', cv='prefit')
    calibrated_model.fit(normalized_calibration_data[RF_best_gene_list].values, normalized_calibration_data['status'])

    # Get calibrated probabilities
    RF_calibrated_probs_train = calibrated_model.predict_proba(X_train)[:, 1]
    RF_calibrated_probs_test = calibrated_model.predict_proba(X_test_train)[:, 1]

    # Instead of saving to Excel, return the probabilities
    return RF_best_model, RF_best_gene_list, RF_calibrated_probs_train, RF_calibrated_probs_test

# Running the analysis and capturing returned probabilities
RF_best_model, RF_best_gene_list, RF_calibrated_probs_train, RF_calibrated_probs_test = run_analysis()