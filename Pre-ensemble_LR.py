...
def run_analysis():
    ...
    # Best genes and hyperparameters for LR
    LR_best_gene_list = ["COX6B1", "TMEM258", "IFI27L2", "MYL6", "SRP14", "TNFAIP2", "GNG5", "DSTN", "FLOT1", "RNF181"]
    best_params = {'penalty': 'l1', 'max_iter': 3808, 'C': 83.65654005254481, 'tol': 0.00251254178413963}

    # Train the best model using the best parameters from TPE
    LR_best_model = LogisticRegression(solver='liblinear', **best_params)
    LR_best_model.fit(normalized_data[LR_best_gene_list].values, normalized_data['status'])
        
    # Prediction and evaluation on the training set
    X_train = normalized_data[LR_best_gene_list].values
    y_train = normalized_data['status']
    predicted_status_train = LR_best_model.predict(X_train) 
    predicted_probs_train = LR_best_model.predict_proba(X_train)[:, 1] 
    normalized_data['Probability'] = predicted_probs_train 

    # Prediction and evaluation on the test set
    X_test_train = normalized_test_data[LR_best_gene_list].values
    y_test_train = normalized_test_data['status']
    test_set_predictions = LR_best_model.predict(X_test_train) 
    test_set_pred_probs = LR_best_model.predict_proba(X_test_train)[:, 1]
    ...
    calibrated_model = CalibratedClassifierCV(LR_best_model, method='sigmoid', cv='prefit')
    calibrated_model.fit(normalized_calibration_data[LR_best_gene_list].values, normalized_calibration_data['status'])

    # Get calibrated probabilities
    LR_calibrated_probs_train = calibrated_model.predict_proba(X_train)[:, 1]
    LR_calibrated_probs_test = calibrated_model.predict_proba(X_test_train)[:, 1]

    # Return the probabilities
    return LR_best_model, LR_best_gene_list, LR_calibrated_probs_train, LR_calibrated_probs_test

# Running the analysis and capturing returned probabilities
LR_best_model, LR_best_gene_list, LR_calibrated_probs_train, LR_calibrated_probs_test = run_analysis()