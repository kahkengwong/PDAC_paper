...
def run_analysis(max_evals, n_startup_jobs, gene_list, csf, csf_name):
    ...
    # Initialize all_metrics_df as an empty DataFrame
    all_metrics_df = pd.DataFrame()
    space = {
        'C': hp.loguniform('C', np.log(0.001), np.log(1000)),
        'penalty_index': hp.choice('penalty_index', range(2)), 
        'max_iter': hp.choice('max_iter', range(100, 5000)), 
    ...
    def objective(params):
        params['penalty'] = penalties[params['penalty_index']]
        del params['penalty_index']
    ...
    # Retrieve and set the correct penalty parameter for best_params
    best_params['penalty'] = penalties[best_params['penalty_index']]
    del best_params['penalty_index']
    ...
    # Correction and conversion of parameters for LR
    best_trial_params_raw = trials.trials[best_trial_index]['misc']['vals']
    best_trial_params_converted = {}
    best_trial_params_converted['penalty'] = penalties[best_trial_params_raw['penalty_index'][0]]
    if 'max_iter' in best_trial_params_raw:
        best_trial_params_converted['max_iter'] = list(range(100, 5000))[best_trial_params_raw['max_iter'][0]]
    if 'C' in best_trial_params_raw:
        best_trial_params_converted['C'] = best_params['C']
    if 'tol' in best_trial_params_raw:
        best_trial_params_converted['tol'] = best_params['tol']

    # Train the best model using the converted parameters
    best_model = LogisticRegression(solver='liblinear', **best_trial_params_converted)
    ...
    # Save all metrics and probabilities to an Excel file
    filename = f"LR_{num_genes}genes_{csf_name}_{max_evals}.xlsx"
