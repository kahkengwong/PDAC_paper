...
def run_analysis(max_evals, n_startup_jobs, gene_list, csf, csf_name):
    ...
    space = {
        ...
        'min_child_samples': hp.choice('min_child_samples', range(20, 30)), 
    ...
    def objective(params):
        model = LGBMClassifier(random_state=1, verbose=-1, **params)
        score = cross_val_score(model, normalized_data[gene_list].values, normalized_data['status'], 
                            scoring=make_scorer(csf), cv=stratified_kfold, n_jobs=-1).mean()
    ...
    # Decode hp.choice outputs to actual values
    best_params['num_leaves'] = range(5, 30)[best_params['num_leaves']]
    best_params['max_depth'] = range(5, 15)[best_params['max_depth']]
    best_params['min_child_samples'] = range(20, 30)[best_params['min_child_samples']]
    if best_params['num_leaves'] <= 1:
        raise ValueError("Invalid value for num_leaves: must be greater than 1")
    ...
    # Iterating through the raw parameters for conversion specifically for LightGBM
    for key, val in best_trial_params_raw.items():
        if key in ['num_leaves', 'max_depth', 'min_child_samples', 'min_split_gain', 'learning_rate', 'subsample', 'colsample_bytree', 'reg_lambda', 'reg_alpha', 'min_child_weight']:
            # For hp.choice parameters, map indices to actual values
            if key == 'num_leaves':
                best_trial_params_converted[key] = range(5, 30)[val[0]]
            elif key == 'max_depth':
                best_trial_params_converted[key] = range(5, 15)[val[0]]
            elif key == 'min_child_samples':
                best_trial_params_converted[key] = range(20, 30)[val[0]]
            # For hp.uniform parameters, the value is directly used
            else:
    ...
    best_model = LGBMClassifier(random_state=1, **best_trial_params_converted)
    ...
    # Extract relevant columns (ID, status, probabilities) from the training and test sets
    ...
    filename = f"GBM_{num_genes}genes_{csf_name}_{max_evals}.xlsx"
