...
def run_analysis(max_evals, n_startup_jobs, gene_list, csf, csf_name):
    ...
    space = {
        'C': hp.uniform('C', 0.01, 1000),
        'gamma': hp.loguniform('gamma', np.log(1e-6), np.log(1e-2)),
    ...
    def objective(params):
        model = SVC(probability=True, random_state=1, class_weight='balanced', **params)
    ...
    # Iterate through the raw parameters and convert to proper format
    for key, val in best_trial_params_raw.items():
        if key == 'kernel':
            best_trial_params_converted[key] = kernel_options[int(np.array(val[0]).astype(int))]
        elif key == 'degree':
            best_trial_params_converted[key] = degree_options[int(np.array(val[0]).astype(int))]
        elif key == 'shrinking':
            best_trial_params_converted[key] = bool(val[0])
        else:
            # For non-categorical parameters that are directly stored as values
            best_trial_params_converted[key] = val[0] if val else None 

    # Train the best model using the converted parameters
    best_model = SVC(probability=True, random_state=1, class_weight='balanced', **best_trial_params_converted)
    ...
    filename = f"SVM_{num_genes}genes_{csf_name}_{max_evals}.xlsx"
